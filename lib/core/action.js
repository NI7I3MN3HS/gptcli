...
const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY||'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',
  baseURL: "https://api-hk.88api.top/v1/",
});
...
async function processCommand(naturalLanguageInput) {
  ...
  try {
    //先翻译
    const translatedInput = await translateToEnglishIfNeeded(naturalLanguageInput);
    spinner.text = '正在生成命令...';
    ...
    while (shouldRegenerate)
    {
      ...
      response = await openai.chat.completions.create({
          ...
          n: 3,        
          messages: [{ role: "system", content: generatePrompt(translatedInput) }]
      });

      // 提取并去重命令行命令建议
      const suggestionsSet = new Set(response.choices.map(choice => {
      ...
      // 标红 DANGEROUS 命令
      const formattedSuggestions = suggestions.map(s => s.includes("DANGEROUS") ? chalk.red(s) : s);
      ...
      {
        // 执行选定的命令
        exec(answer.selectedCommand, { encoding: 'buffer' },(errorBuffer,stdoutBuffer, stderrBuffer) => {
...
async function translateToEnglishIfNeeded(input) {
    ...
    if (isChinese(input)) {
      const translation = await openai.chat.completions.create({
          model: 'gpt-3.5-turbo',
          messages: [{ role: "system", content: `Translate the following Chinese text to English:\n\n${input}\n\nTranslation:` }],
      });
