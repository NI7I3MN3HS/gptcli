...
async function processCommand(naturalLanguageInput) {
  ...
  try {
    ...
    while (continueExecution) {
      let { action, isDangerous } = await askForAction(suggestion);

      if (action === "✅直接执行") {
        if (isDangerous) {
          suggestion = cleanCommand(suggestion); // 清除 'DANGEROUS' 文本
        }
...
// 检查命令是否包含 DANGEROUS 标记
function containsDangerous(command) {
...
async function askForAction(command) {
  ...
  let isDangerous = containsDangerous(command);

  if (isDangerous) {
    displayCommand = chalk.red("DANGEROUS:"+cleanCommand(command));
  }
